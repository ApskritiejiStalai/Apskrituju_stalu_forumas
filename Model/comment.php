...
class comment {
    ...
    public function TakeComment($module_id/*, $offset, $limit*/) {
        include 'db.php';
...
//                . "LIMIT {$offset}, {$limit}"; 
    ...
    public function ElementsCount($module_id){
        ...
        $result = mysqli_query($connect,$query);
        foreach($result as $key=>$val) $total = $val['kiekis'];
        return $total;
    }
    ...
    public function InsertComment($module_id, $comment) {
        ...
        foreach($result as $key=>$val) $comment_id = $val['Komentaro_id'];
    ...
    public function upvoteExist($module_id){
        ...
        $result = mysqli_query($connect, $query);   
        if(!is_bool($result)){
            foreach($result as $key=>$val) $vote_id = $val['Up_id'];
        }
    ...
    public function getComment($module_id){
        ...
        $result = mysqli_query($connect,$query);
        if(!is_bool($result)){
            foreach($result as $key=>$val) $text = $val['Komentaras'];
        }
        if (mysqli_num_rows($result) != 0) {
            return $text;
        }
